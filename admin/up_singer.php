    ...
    <div id="layout" class="pure-g">
        ...
        <div class="content pure-u-1 pure-u-md-21-24">
            <div class="header-small">
                <div class="items">
                    <?php include "includes/insert_singer.php" ?>
                    ...
                    <form action="" method="post" autocomplete="off" enctype="multipart/form-data" class="pure-form pure-form-stacked">
                        <fieldset>
                            ...
                            <select id="origin" name="origin_id" class="pure-input-1">
                                <?php 
                                    $get_origin ="select * from origin";

                                    $run_origin = mysqli_query($conn, $get_origin);

                                    while($row_origin=mysqli_fetch_array($run_origin)){
                                    $origin_id = $row_origin['id'];

                                    $origin_title = $row_origin['name'];

                                    echo "<option value='$origin_id'>$origin_title</option>";
                                    }
                                ?>
                            </select>

                            <input type="hidden" name="id" value="1">
                            <button  name="insert_singer" value="Add Singer" class="pure-button button-success">Save</button>
                        </fieldset>
                    </form>
                </div>    
            </div>
